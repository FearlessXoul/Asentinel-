...
// Stack: Node.js + Express + Stripe + JWT + SQLite (swap to Postgres on prod)
...
const db = new Database("asentinel.db");

// ─── ENV (set these in Railway) ───────────────────────────────────────────────
// ANTHROPIC_API_KEY
// STRIPE_SECRET_KEY
// STRIPE_WEBHOOK_SECRET
// JWT_SECRET                (random long string)
// STRIPE_PRO_PRICE_ID       (from Stripe dashboard)
// STRIPE_ELITE_PRICE_ID     (from Stripe dashboard)
...
// Per-agent access control by plan
const AGENT_ACCESS = {
  free: ["oracle", "shrink"],
...
app.post("/auth/signup", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: info.lastInsertRowid, email }, process.env.JWT_SECRET, { expiresIn: "30d" });
...
// ─── CLAUDE PROXY ─────────────────────────────────────────────────────────────
const AGENT_SYSTEMS = {
  oracle: `You are Oracle, an elite market analyst. Analyze markets across Forex, Crypto, Stocks, Options, and Futures with precision. Format: Bias / Key Levels / Confluences / Trade Plan / Risk.`,
  ...
  quant: `You are Quant, a quantitative analyst. Help with win rate math, expectancy, position sizing (Kelly, fixed fractional), backtesting logic.`,
...
app.post("/chat", requireAuth, async (req, res) => {
  ...
  if (user.plan === "free") {
    ...
    if (user.daily_reset_date !== today) {
      db.prepare("UPDATE users SET daily_message_count = 0, daily_reset_date = ? WHERE id = ?").run(today, user.id);
    ...
    db.prepare("UPDATE users SET daily_message_count = daily_message_count + 1 WHERE id = ?").run(user.id);
  }

  // Save user message
  const lastMsg = messages[messages.length - 1];
  db.prepare("INSERT INTO chats (user_id, mode, role, content) VALUES (?, ?, ?, ?)").run(user.id, mode, lastMsg.role, lastMsg.content);
  ...
  try {
    ...
    db.prepare("INSERT INTO chats (user_id, mode, role, content) VALUES (?, ?, ?, ?)").run(user.id, mode, "assistant", reply);
...
app.get("/chat/:mode", requireAuth, (req, res) => {
  const rows = db.prepare(
    "SELECT role, content FROM chats WHERE user_id = ? AND mode = ? ORDER BY created_at ASC LIMIT 100"
  ).all(req.user.id, req.params.mode);
  res.json(rows);
});

// Clear chat
app.delete("/chat/:mode", requireAuth, (req, res) => {
  db.prepare("DELETE FROM chats WHERE user_id = ? AND mode = ?").run(req.user.id, req.params.mode);
  res.json({ ok: true });
});

// ─── POSITIONS ────────────────────────────────────────────────────────────────
app.get("/positions", requireAuth, (req, res) => {
  const rows = db.prepare("SELECT data FROM positions WHERE user_id = ? ORDER BY created_at DESC").all(req.user.id);
  res.json(rows.map((r) => JSON.parse(r.data)));
});

app.post("/positions", requireAuth, requirePlan("pro", "elite"), (req, res) => {
  const pos = req.body;
  db.prepare("INSERT OR REPLACE INTO positions (id, user_id, data) VALUES (?, ?, ?)").run(pos.id, req.user.id, JSON.stringify(pos));
  res.json({ ok: true });
});

app.delete("/positions/:id", requireAuth, (req, res) => {
  db.prepare("DELETE FROM positions WHERE id = ? AND user_id = ?").run(req.params.id, req.user.id);
  res.json({ ok: true });
});

// ─── TRADE LOG ────────────────────────────────────────────────────────────────
app.get("/tradelog", requireAuth, (req, res) => {
  const rows = db.prepare("SELECT data FROM trade_log WHERE user_id = ? ORDER BY created_at DESC LIMIT 100").all(req.user.id);
  res.json(rows.map((r) => JSON.parse(r.data)));
});

app.post("/tradelog", requireAuth, (req, res) => {
  db.prepare("INSERT INTO trade_log (user_id, data) VALUES (?, ?)").run(req.user.id, JSON.stringify(req.body));
  res.json({ ok: true });
});
...
// Stripe webhook — keep subscriptions in sync
app.post("/webhook/stripe", (req, res) => {
  ...
  if (event.type === "customer.subscription.updated" || event.type === "customer.subscription.created") {
    ...
    const customer = db.prepare("SELECT id FROM users WHERE stripe_customer_id = ?").get(sub.customer);
    if (customer) {
      db.prepare("UPDATE users SET plan = ?, stripe_subscription_id = ? WHERE id = ?").run(plan, sub.id, customer.id);
    }
  }
  ...
  if (event.type === "customer.subscription.deleted") {
    ...
    const customer = db.prepare("SELECT id FROM users WHERE stripe_customer_id = ?").get(sub.customer);
    if (customer) {
      db.prepare("UPDATE users SET plan = 'free' WHERE id = ?").run(customer.id);
    }
  }
