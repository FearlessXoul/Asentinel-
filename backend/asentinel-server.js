// ════════════════════════════════════════════════════════════════════════════
// ASENTINEL BACKEND — asentinel-server.js
// Stack: Node.js + Express + PostgreSQL + JWT + Stripe + Anthropic
// Deploy: Render.com
// ════════════════════════════════════════════════════════════════════════════
...
async function initDB() {
  ...
  `);
  // Migrate: add trial_ends_at if missing
...
const AGENT_ACCESS = {
  free:  ["oracle", "shrink"],
  trial: ["oracle", "sniper", "shrink", "autopsy", "quant", "risk", "news"],
...
function getEffectivePlan(user) {
  ...
  return onTrial ? "trial" : user.plan;
}
...
app.get("/chat/:mode", requireAuth, async (req, res) => {
  const rows = await db.all("SELECT role, content FROM chats WHERE user_id = $1 AND mode = $2 ORDER BY created_at ASC LIMIT 100", [req.user.id, req.params.mode]);
  res.json(rows);
});
...
app.post("/positions", requireAuth, async (req, res) => {
  const pos = req.body;
  await pool.query("INSERT INTO positions (id, user_id, data) VALUES ($1, $2, $3) ON CONFLICT (id) DO UPDATE SET data = $3", [pos.id, req.user.id, JSON.stringify(pos)]);
  res.json({ ok: true });
});
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      if (session.metadata?.userId) {
        await pool.query("UPDATE users SET stripe_subscription_id = $1 WHERE id = $2", [session.subscription, session.metadata.userId]);
      }
    }
    if (["customer.subscription.updated", "customer.subscription.created"].includes(event.type)) {
      ...
      if (customer) await pool.query("UPDATE users SET plan = $1, stripe_subscription_id = $2 WHERE id = $3", [plan, sub.id, customer.id]);
    }
    if (event.type === "customer.subscription.deleted") {
      const sub = event.data.object;
      const customer = await db.get("SELECT id FROM users WHERE stripe_customer_id = $1", [sub.customer]);
      if (customer) await pool.query("UPDATE users SET plan = 'free' WHERE id = $1", [customer.id]);
    }
...
// ─── HEALTH ───────────────────────────────────────────────────────────────────
app.get("/health", (_, res) => res.json({ status: "ok", service: "asentinel-api", time: new Date().toISOString() }));
